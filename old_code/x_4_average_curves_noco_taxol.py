...
def main(config_path):
    ...
    for file in os.listdir(folder):
        if file.endswith('.h5'):
            ...
            df_all = pd.read_hdf(folder+file)
            # filter tracks
    ...
    df_all_tracks['USABLE'] = np.ones(len(df_all_tracks))
    df_all_tracks.sort_values(by='FRAME')
    for file in df_all_tracks['file'].unique():
        
        df_file = df_all_tracks[df_all_tracks['file']==file]

        for track in df_file['TRACK_ID'].unique():
            df_track = df_file[df_file['TRACK_ID']==track]
