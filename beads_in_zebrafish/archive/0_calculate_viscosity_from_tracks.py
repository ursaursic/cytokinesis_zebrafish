...
This code calculates effective viscosities from the trackmate csv files.
...
# Analyze data from "magnetic_tweezers" folder
...
analysis_dir = '/Volumes/cytokinesis-zebrafish-collab/magnetic_tweezers/2_analysis/'
...
def main():
    ...
    for idx in tqdm(range(len(df_general_info))):
        ...
        phases = df_general_info[['m_phase_1_start (frame)', 'm_phase_1_end (frame)', 
        ...
        calculate_displacement(df, first_pulse, t_on, t_off, substract_background=substract_background) # calculate displacement for every pulse (option: substract background flows)

        if intermediate_plots:
            plot_trajectories(filename, df, first_pulse, t_on, t_off, save_to_filepath=f'{plots_dir}intermediate_plots/trajectories_{filename}.html', show_background_fit=True)
            ...
            plot_viscoelastic_responce(filename, df, dt, first_pulse, t_on, t_off, save_to_filepath=plots_dir)
        ...
        # all results so far
        df_all_results = get_results_mgtw(filename, df, magnet_pulses, df_all_results)

    if substract_background:
        df_all_results.to_csv(f'{analysis_dir}results_viscosity_corrected.csv')
    else:
