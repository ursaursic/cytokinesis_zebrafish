...
def main(config_path):
    ...
    for measurement in df_results_filtered[['file', 'TRACK_ID', 'PULSE_NUMBER']].values:
        ...
        if len(df['POSITION_T'])>50 or len(df[df['MAGNET_STATUS']==1])<10 or len(df[df['MAGNET_STATUS']==0])<20:
            plt.plot(df['POSITION_T']-np.min(df['POSITION_T']), df[displacement_column], 'o', color=sns.color_palette('tab10')[0], alpha=0.5, markersize = 4)
    ...
    for force_range in force_ranges:
        ...
        for (mt_status, label, i) in zip(['y', 'n'], ['Interphase', 'M-phase'], [0, 1]):
            ...
            # plt.plot(time_grouped, displacement_grouped, '.', color=sns.color_palette('tab10')[i], label= label)

            # plt.fill_between(
            # time_grouped,
            # displacement_grouped - displacement_grouped_std, 
            # displacement_grouped + displacement_grouped_std,
            # color=sns.color_palette('tab10')[i], alpha=0.2)
            ...
            for file in df_force_range['file'].unique():
                for track in df_force_range[df_force_range['file']==file]['TRACK_ID'].unique():
                    for pulse in df_force_range[(df_force_range['file']==file)&(df_force_range['TRACK_ID']==track)]['PULSE_NUMBER'].unique():
                        ...
                        if not len(df[df['MAGNET_STATUS']==1]) == 10 or not len(df[df['MAGNET_STATUS']==0]) == 30:
                            continue
                        if df['MT_STATUS'].unique() == 'y':
                            plt.plot(df['NORMALIZED_TIME'], df[displacement_column], '-', color=sns.color_palette('tab10')[0], alpha=0.02, zorder=1)
                            all_tracks_iphase.append(df[displacement_column].values)
                        elif df['MT_STATUS'].unique() == 'n':
                            plt.plot(df['NORMALIZED_TIME'], df[displacement_column], '-', color=sns.color_palette('tab10')[1], alpha=0.02, zorder=1)
                            all_tracks_mphase.append(df[displacement_column].values)

        average_i, ci_i = weighted_bootstrap(np.array(all_tracks_iphase))
        plt.plot(time_grouped, average_i, '.-', color=sns.color_palette('tab10')[0], label= 'Interphase')
        plt.fill_between(time_grouped, ci_i[0].astype(float), ci_i[1].astype(float), color=sns.color_palette('tab10')[0], alpha=0.3)

        average_m, ci_m = weighted_bootstrap(np.array(all_tracks_mphase))
        plt.plot(time_grouped, average_m, '.-', color=sns.color_palette('tab10')[1], label= 'M-phase')
        plt.fill_between(time_grouped, ci_m[0].astype(float), ci_m[1].astype(float),color=sns.color_palette('tab10')[1], alpha=0.3)
    ...
    # Absolutely all tracks together
    for force_range in force_ranges:
        df_force_range = df_all_tracks[(df_all_tracks['AVG_FORCE']>=force_range[0])&(df_all_tracks['AVG_FORCE']<force_range[1])&(df_all_tracks['USABLE']==1)]
                    ...
                    for track in df_force_range[df_force_range['file']==file]['TRACK_ID'].unique():
                        for pulse in df_force_range[(df_force_range['file']==file)&(df_force_range['TRACK_ID']==track)]['PULSE_NUMBER'].unique():
                            ...
                            if len(df['MT_STATUS']) == 0:
                                continue
                            if df['MT_STATUS'].unique() == 'y':
                                plt.plot(df['NORMALIZED_TIME'], df[displacement_column], '-', color=sns.color_palette('tab10')[0], alpha=0.05)
                            elif df['MT_STATUS'].unique() == 'n':
                                plt.plot(df['NORMALIZED_TIME'], df[displacement_column], '-', color=sns.color_palette('tab10')[1], alpha=0.05)
